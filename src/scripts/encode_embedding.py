...
def save_video_embedding(session, video_name, video_path, video_vector, text_vectors, face_video_vector, text_probs):
    ...
    if existing_video:
        ...
        existing_video.face_video_vector = face_video_vector
    else:
        # Create new record
        video = Video(
            name=video_name,
            ...
            face_video_vector = face_video_vector,
        )
        session.add(video)
    session.commit()
    session.close()
...
def process_video(es, session, video_path, text_vectors, text_probs):
    ...
    face_video_vector = model.extract_video(video_path, crop_face=True, reduction="max").cpu()
...
if __name__ == "__main__":
    ...
    # with open("../data/groundtruth_face_crops.json", "r") as f:
    #     gt_json = json.load(f)
    # priority_videos = [video.replace(".mp4", "") for video in gt_json.keys()]
    import tqdm
    i = 0
...
# python scripts/encode_embedding.py  --video_dir ../data/faces --checkpoint ./pretrain_clipvip_base_16.pt --json_dir ../data/kaggle/working/output
